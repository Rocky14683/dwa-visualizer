...
impl Robot {
    ...
    pub fn predict(&self, left_v: f64, right_v: f64, dt: f64) -> (Pose, Motion) {
        ...
        println!("left_v: {}, left_v_round: {}", left_v, left_v_rounded);
        println!("right_v: {}, right_v_round: {}", right_v, right_v_rounded);
        ...
        if left_v_rounded == right_v_rounded {
            ...
            // pure turn motion
            new_pose.orientation = self.pose.orientation + ((right_v_rounded - left_v_rounded) * dt / (2.0 * self.size_radius));
            return (new_pose, Motion::ROTATION());
        }
        // general motion
        let r = self.size_radius * (left_v + right_v) / (right_v - left_v);
        let d_theta = (right_v - left_v) * dt / (2.0 * self.size_radius);
        let centre = Point { x: self.pose.position.x - r * self.pose.orientation.sin(), y: self.pose.position.y + r * self.pose.orientation.cos() };
        ...
        let start_angle = if r > 0.0 {
            self.pose.orientation - std::f64::consts::PI / 2.0
        } else {
            self.pose.orientation + std::f64::consts::PI / 2.0
        };

        let stop_angle = start_angle + d_theta;
        return (new_pose, Motion::GENERAL(centre, r, start_angle, stop_angle));
    }
    ...
    pub fn draw_dwa_path(&self, handle: &mut RaylibDrawHandle, color: Color) {
        for motion in self.dwa_path.clone() {
            println!("{:?}", motion);
            match motion {
                Motion::GENERAL(centre, radius, angle_1, angle_2) => {
                    ...
                    // }
                    // handle.draw_circle_sector(centre.to_vector2(),
                    //                                 radius as f32,
                    //                                 angle_1.to_degrees() as f32,
                    //                                 angle_2.to_degrees() as f32, 10, color);

                    handle.draw_circle_lines(centre.x as i32, centre.y as i32, radius as f32, color);
        ...
        for left_v in left_possible_v {
            for right_v in right_possible_v {
                if left_v <= self.max_velocity && left_v >= -self.max_velocity && right_v <= self.max_velocity && right_v >= -self.max_velocity {
                    ...
                    let heading = new_pose.position.get_angle(target.clone());

                    let distance_to_barrier = barriers.get_shortest_to_barrier(new_pose.clone(), self.size_radius);
                    ...
                    let heading_cost = weight.angle_weight * (new_pose.orientation - heading);

                    let total_benefit = distance_benefit - obstacle_cost - heading_cost;
